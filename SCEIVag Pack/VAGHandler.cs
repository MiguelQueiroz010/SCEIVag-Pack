...
namespace SCEIVag_Pack
{
    ...
    public static class ADPCM
    {
        ...
        public static byte[] PCMtoWAV(byte[] PCM, int freq, int channels)
        {
            var b = new List<byte>();
            int size = PCM.Length + 0x26;//Começando do Wavefmt
            b.AddRange(Encoding.Default.GetBytes("RIFF"));
            b.AddRange(BitConverter.GetBytes((UInt32)size));
            b.AddRange(Encoding.Default.GetBytes("WAVEfmt "));
            byte[] predef = new byte[] { 0x12, 00, 00, 00, 0x01, 00 };
            b.AddRange(predef);
            b.Add((byte)channels);
            b.Add(0);
            b.AddRange(BitConverter.GetBytes((UInt32)freq));
            b.AddRange(BitConverter.GetBytes((UInt32)freq*2));
            byte[] predef2 = new byte[] { 0x02, 00, 0x10, 00, 00, 00, 0x64, 0x61, 0x74, 0x61 };
            b.AddRange(predef2);
            b.AddRange(BitConverter.GetBytes((UInt32)PCM.Length));

            b.AddRange(PCM);

            //WAVE STRUCT
            //0: RIFF magic
            //4: WaveSec Size(uint32)
            //8: WAVEfmt  Section
            //etc
            //
            //
        ...
        public static byte[] WAVtoPCM(byte[] WAV, int freq, out bool diffreq, out int wfreq)
        {
            var b = new List<byte>();
            bool freqx = false;
            ...
            if (freq != frequency)
            {
                MessageBox.Show("A frequência do áudio importado é: " + frequency.ToString()+"Hz"
                    + "\nSó que o áudio original tem frequência de: " + freq.ToString()+"Hz", "Opa, pera aí!");
                freqx = true;
